...
import { oauthRequest, oauthRequestFull } from "./auth/netsuiteOAuthClient.js"; // adjust path as needed
// -----------------------Hubspot Imports-----------------------
import {
...
import { filterOutOwnEchoes } from "../utils/loopGuard.js";
import { markSyncedToNetsuite } from "../utils/loopGuard.js";
...
//   try {
//     if (netsuiteId) {
//       // UPDATE: Use PATCH - no id to extract, regular oauthRequest is fine
...
//       };
//     } else {
//       // CREATE: Use POST - need oauthRequestFull to read the Location header
//       const url = `${baseUrl}${basePath}`;
//       logger.info("Creating new NetSuite Customer...");

//       const res = await oauthRequestFull("POST", url, mappedPayload);

//       const locationHeader = res?.headers?.location || res?.headers?.Location;

//       let newCustomerId = null;
//       if (locationHeader) {
//         newCustomerId = locationHeader.split("/").pop();
//       }

//       if (!newCustomerId) {
//         logger.warn(
//           `[NetSuite] Customer created but could not extract id from Location header: ${locationHeader}`
//         );
//       }
...
async function upsertNetSuiteCustomer(mappedPayload, netsuiteId = null) {
  ...
  try {
    if (netsuiteId) {
      ...
      // --- Loop-guard: stamp immediately after a successful write ---
      await markSyncedToNetsuite(netsuiteId);
      ...
      if (!newCustomerId) {
        ...
        // --- Loop-guard: stamp the newly created record too ---
        // Important for CREATE: the NS -> HS delta pull will see this brand-new
        // record's lastmodifieddate on its very next run. Without this stamp,
        // it would look like a "genuine NetSuite-side change" and get pushed
        // straight back to HubSpot as if NetSuite created it.
        await markSyncedToNetsuite(newCustomerId);
      }
...
async function sync_netsuite_customers_to_hubspot_companies() {
  try {
    ...
    for await (const { records, stats } of customerStream) {
      try {
        // --- Loop-guard: drop records that are just echoes of our own HS -> NS writes ---
        const genuineRecords = await filterOutOwnEchoes(records);

        if (genuineRecords.length === 0) {
          logger.info(
            `[Netsuite-Hubspot] All ${records.length} record(s) in this page were own-echoes, skipping batch.`
          );
          continue;
        }
        ...
        if (genuineRecords.length < records.length) {
          logger.info(
            `[Netsuite-Hubspot] Filtered ${
              records.length - genuineRecords.length
            } echo(es) out of ${records.length} record(s).`
          );
        }

        await processBatchOfCompanies(genuineRecords);

        logger.info(
          `[Netsuite-Hubspot Progress] : ${stats.page}, ${stats.totalProcessed}, ${stats.recordsPerSecond}`
        );
      } catch (error) {
  ...
  try {
    ...
    for await (const { records, stats } of customerStream) {
      try {
        ...
        if (genuineRecords.length === 0) {
          logger.info(
            `[Netsuite-Hubspot] Person: all ${records.length} record(s) in this page were own-echoes, skipping batch.`
          );
          continue;
        }
        ...
        if (genuineRecords.length < records.length) {
          logger.info(
            `[Netsuite-Hubspot] Person: filtered ${
              records.length - genuineRecords.length
            } echo(es) out of ${records.length} record(s).`
          );
        }

        await processBatchOfContacts(genuineRecords);

        logger.info(
          `[Netsuite-Hubspot Progress] Person: ${stats.page}, ${stats.totalProcessed}, ${stats.recordsPerSecond}`
        );

        // NOTE: removed the debug `return;` that was here - it was cutting the
        // loop short after the first page, so only page 1 was ever synced.
      } catch (error) {
        logger.error(`Error in syncing netsuite customers to hubspot:`, {
          status: error?.status,
          response: error.response?.data,
