...
// This function is used to retrieve contact from hubspot and sync it as company in netsuite
async function sync_hubspot_contacts_to_netsuite_contacts() {
  try {
    const lastSyncTime = "2026-07-01T10:00:00.000Z";
    ...
    for await (const { records, stats } of contactStream) {
      ...
      await processContactsToNetsuite(records[0]);
...
async function processContactsToNetsuite(hs_contact) {
  try {
    logger.info(`Contact: ${JSON.stringify(hs_contact, null, 2)}`);
    const hs_client = getHubspotClient();
    let sourceId = null;
    if (hs_contact) {
    ...
    const payload = contactMappingHSToNS(hs_contact.properties);
    ...
    logger.error("Error processing Hubspot Contact", {
      message: error.message,
      status: error.response?.status,
      method: error.config?.method?.toUpperCase(),
      url: error.config?.url,
      // Safely grab the specific API error string rather than the whole payload
      api_error:
        error.response?.data?.message ||
        error.response?.data?.error ||
        "Check full logs, payload omitted",
      stack: error.stack,
    });
  }
}
async function sync_hubspot_companies_to_netsuite_companies() {
  try {
    const lastSyncTime = "2026-07-01T10:00:00.000Z";
    ...
    for await (const { records, stats } of contactStream) {
      logger.info(`Record : ${JSON.stringify(records[0], null, 2)}`);
      await processCompaniesToNetsuite(records[0]);
      logger.info(`[Hubspot Progress] ${endpoint}`, {
        page: stats.page,
        processed: stats.totalProcessed,
        speed: `${stats.recordsPerSecond} rec/sec`,
...
async function processCompaniesToNetsuite(hs_company) {
  try {
    logger.info(`Companies: ${JSON.stringify(hs_company, null, 2)}`);
    const hs_client = getHubspotClient();
    let sourceId = null;
    if (hs_company) {
    ...
    const payload = companyMappingHSToNS(hs_company.properties);

    logger.info(`Payload: ${JSON.stringify(payload, null, 2)}`);

    // Insert as Companies in NetSuite
    const netsuiteCustomer = await upsertNetSuiteCustomer(payload, sourceId);
    logger.info(
      `NetSuite Customer: ${JSON.stringify(netsuiteCustomer, null, 2)}`
    );
    ...
    if (!sourceId && netsuiteCustomer?.netsuiteId) {
      // Update Contact
      const HSContactUpdate = await hs_client.companies.updateCompany(
        hs_company.id,
        {
          // properties: {
          sourceid: netsuiteCustomer.netsuiteId,
      ...
      logger.info(
        `Hubspot Company Updated Successfully: ${JSON.stringify(
          HSContactUpdate,
          null,
          2
        )}`
...
export {
  ...
  sync_hubspot_contacts_to_netsuite_contacts,
  processContactsToNetsuite,
  sync_hubspot_companies_to_netsuite_companies,
};
