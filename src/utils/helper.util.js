...
function customerQuery(arg) {
  ...
  const query = `
    SELECT
      ...
      c.stage,
      TO_CHAR(c.lastmodifieddate, 'YYYY-MM-DD HH24:MI:SS') AS lastmodifieddate,
      c.dateclosed,
      c.firstsaledate,
    ...
    -- NOTE: lastmodifiedby filter intentionally removed - we cannot rely on it
    -- (integration user isn't reliably attributed on every write path in NetSuite).
    -- Loop prevention instead relies entirely on lastmodifieddate + the
    -- syncStateStore/loopGuard stamp (see filterOutOwnEchoes in loopGuard.js),
    -- which compares each record's lastmodifieddate against the timestamp we
    -- stamped immediately after our own HS -> NS write.
